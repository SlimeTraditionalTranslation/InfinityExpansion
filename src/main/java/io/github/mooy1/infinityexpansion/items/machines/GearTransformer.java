...
public final class GearTransformer extends AbstractEnergyCrafter implements RecipeDisplayItem {
    ...
    public void update(@Nonnull BlockMenu inv) {
        ...
        if (inputItem == null) { //no input
            ...
            inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(Material.BLUE_STAINED_GLASS_PANE, "&9放入工具或裝備"));
            return;

        }

        if (!SF && StackUtils.getId(inputItem) != null) {
            inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(Material.RED_STAINED_GLASS_PANE, "&c黏液科技物品可能不會改變它的材料!"));
            return;
        }
        ...
        if (inputToolType == null) { //invalid input
            ...
            inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(Material.BARRIER, "&c不是工具或裝備!"));
            return;
        ...
        if (pair == null) { //invalid material
            ...
            inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(Material.BARRIER, "&c無效材料!"));
            return;
        ...
        inv.replaceExistingItem(STATUS_SLOT, new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, "&a工具已改變!"));
    }
